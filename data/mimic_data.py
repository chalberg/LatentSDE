...
def query_raw_mimiciv(project):
    q = """
      ...
      from merged_table
      """
    logging.info("Querying MIMIC-IV. This may take a moment...")
    df = pd.read_gbq(query=q, project_id=project)
    df.to_csv('raw/mimic-iv/raw_mimic.csv')


def build_raw_mimiciv():
    logging.root.setLevel(logging.INFO)

    raw_folder = Path("raw/mimic-iv")
    ...
    # For saving
    folder = raw_folder / "reduced_feature_dataframes"
...
def process_interpolate_and_save(name, labels, data_dir, test=False):
    # Location setup
    raw_folder = data_dir / "raw/mimic-iv"
    processed_folder = data_dir / "processed/MIMIC-IV"
    if not os.path.isdir(processed_folder):
        os.mkdir(processed_folder)
    
    # Handle save locations
    save_folder = processed_folder / name
    if not os.path.isdir(save_folder):
        os.mkdir(save_folder)
    fname = save_folder / "improved-neural-sde_data{}.npz".format(
        "_test" if test else ""
    )
    ...
    # exclusions and label processing
    static_data, temporal_data, labels = perform_exclusions(name, static_data, temporal_data, labels)
    ...
    # Save
    np.savez(fname, **processed_data, labels=labels)
...
if __name__=="__main__":
    ...
    parser.add_argument('--test', type=bool, default=False, help='Build test dataset')

    args = parser.parse_args()
    torch.manual_seed(args.seed)

    # query MIMIC-IV
    query_raw_mimiciv(project=args.project)

    # Load
    raw_mimic = combine_and_dump()
    static, temporal = build_raw_mimiciv(raw_mimic)

    # Convert to Numpy
    convert_to_numpy(static, temporal)

    # Processing
    data_dir = ''
    raw_folder = 'raw\mimic-iv'
    npz = np.load(raw_folder / "reduced_format.npz", allow_pickle=True)
    named_labels = [("Sepsis", common.open_npz(npz, "sepsis_data"),
                     "Mortality", common.open_npz(npz, "mortality_data"))]
    
    for name, labels in tqdm(named_labels, desc="Processing and interpolating."):
      process_interpolate_and_save(name, labels, data_dir, test=args.test)